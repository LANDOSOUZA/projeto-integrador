...
async function reporEstoque(pedido) {
  try {
    // pega o primeiro produto do pedido
    const produtoId = pedido.itens[0]?.produtoId?._id
    if (!produtoId) throw new Error('ProdutoId não encontrado no pedido')

    // chama o store para repor estoque e atualizar pedido
    ...
    <aside class="w-64 bg-[#005CA9] text-white flex flex-col p-4">
      ...
      <nav class="flex flex-col gap-3">
        ...
        <button @click="irParaConfiguracoes"
          ...
          <button @click="abaAtiva = 'estoque'"
                class="text-left p-2 rounded"
                :class="abaAtiva === 'estoque' ? 'bg-blue-800' : 'hover:bg-blue-700'">
      📦 Estoque
        </button>
      </nav>
    </aside>
            ...
            <div class="space-y-1">
              ...
              <div class="text-sm">
                ...
                <<span v-if="Array.isArray(pedido.itens)">
                  {{ pedido.itens.map(i => `${i.produtoId?.nome} x${i.quantidade}`).join(', ') }}
                </span>
              </div>
              <!-- Botões de ação -->
              <div class="flex gap-2">
